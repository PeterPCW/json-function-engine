...
- **Skip Regex Validation Option**
  - New `skipRegexValidation` option for `EngineOptions` and `ExecutionContext`
  - Allows skipping ReDoS pattern validation at execute time
  - Useful for complex regex patterns that trigger false positives in validation
  - When enabled, bypasses regex cache for safety
  - Can be set at engine initialization or per-execution

- **Silent Mode**
  - New `silent` option to suppress all console logging
  - Useful for CLI tools that don't want noisy output
  - Uses no-op logger internally
  - Works with best-effort error handling mode

### Changed

- `compileRegex()` and `validateRegexPattern()` now accept optional `skipValidation` parameter
